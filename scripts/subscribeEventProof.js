        ...
        try {
            await api.rpc.ethy.getEventProof(eventId, async (versionedEventProof) => {
                ...
                if (eventProof) {
                    ...
                    if (lastValidatorsSet && lastValidatorsSet.id === eventProof.validatorSetId.toString() && JSON.stringify(newValidators) === JSON.stringify(lastValidatorsSet.validators)){
...
function getValidatorAddedToBridge(bridge) {
    ...
    ethers.provider.getLogs(filter).then((result) => {
        if (result.length > 0) {
            ...
            logger.info(`Previous Validator set in bridge is: ${lastValidatorsSet}`);
        }
    });
